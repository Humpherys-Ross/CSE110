# I have added a death and game over scenario to the game. There is one option
# in each branch that will lead to a game over.
# I have also added a restart function that will restart the game if the user
# makes an invalid choice or reaches a game over scenario.

# Create your own text-based adventure game with at least three levels of
...
# (This means that every time the user plays the game they should answer at
# least three questions because every branch of the game should go three
# levels deep.)

# Scenarios that follow an earlier answer should be handled with nested if
# statements. In other words, the body/block of the first if statement will
# contain a print statement and then another if statement inside it.
...
# regardless of the uppercase/lowercase used in the response (e.g., "match",
# "MATCH", "Match" should all work).

# Making different choices should take you to different scenarios. (You
# shouldn't have the same result for different choices.)
...
# resulted in choices Follow/Look, you should not be able to respond with
# "Follow" to the question that asked for Run/Hide.

# A good way to accomplish this is to have a series of nested if statements.
# (That is, the print and then the next if statement will be within the
# body/block of the first if statement.)

# For each question, you should provide an "else" clause to handle the case
# that the user tries to type something other than the possible choices.
# It is up to you how to handle this case.

def main_game():
    # Starting point of the game.
    print("\nWelcome to the 'Escape from Draken Castle'!\n")
    print("You are imprisoned in the infamous Draken Castle. The only light "
          "filtering in is from the bars in a high window.")
    # Level 1 Choices
    print("\n1) What will you do?")
    print("- DIG beneath the floor using your spoon.")
    print("- WAIT for the guard to doze off.")
    print("- SING a haunting tune.")
    # get user input and convert to uppercase so we get a match regardless of
    # case
...
def dig_branch():
    ...
    # Level 2 Branches
    if response == "FOLLOW":
        print("\nThe tunnel leads outside, but a "
              "pitfall catches you off-guard.")
        # call the game_over function to restart the game
        game_over()
    elif response == "GO":
        print("\nYou stumble upon an underground chamber filled with glowing "
              "gems.")
        # Level 3 Choices
        print("\n3) What do you do?")
        print("- TOUCH a gem.")
        ...
        # get user input and convert to uppercase so we get a match regardless
        # of case
        response = input().upper()

        # Level 3 Branches
        if response == "TOUCH":
            print("\nThe gem emits a blinding light and you vanish. "
                  "Game Over.")
            # call the game_over function to restart the game
            game_over()
        elif response == "EXIT":
            print("\nYou cautiously exit the chamber, making your way to "
                  "freedom!")
        elif response == "HIDE":
        ...
        print("\nThe tunnel is filled with traps. You narrowly dodge them and "
              "find a way out!")
    else:
        # if the user enters an invalid choice, restart the game
        print("\nInvalid choice. Restarting the adventure.")
...
def wait_branch():
    ...
    if response == "GRAB":
        ...
        if response == "UNLOCK":
            ...
            print("\nThe prisoners rally together and overpower the guards. "
                  "Freedom is yours!")
        elif response == "THROW":
...
def sing_branch():
    ...
    # Level 2 Branches
    if response == "ASK":
        print("\nThe maid, moved by your plight, offers you a hidden escape "
              "route.")
        # Level 3 Choices
        ...
        if response == "FOLLOW":
            ...
            print("\nTouched by your kindness, she helps you find the safest "
                  "way out.")
        else:
            # if the user enters an invalid choice, restart the game
            print("\nInvalid choice. Restarting the adventure.")
