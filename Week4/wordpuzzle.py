...
# 2 Add a check to verify that the length of the guess is the same as the
# length of the secret word. If it is not, display a message. If they are the
# same, then proceed to give the hint.
...
#   Letters that are present in the secret word, but in a different spot
#   (lowercase).

#   Letters that are present in the secret word at that exact spot spot
#   (uppercase).
...
def wordpuzzle():
    ...
    while secret_word != answer and guess_count < total_guesses:
        ...
        # Check if the length of the guess is the same as the length of the
        # secret word
